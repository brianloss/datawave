...
public class GlobalIndexUidAggregatorTest {
    ...
    @Test
    public void testPropagateRemoveAndAdd() throws Exception {
        agg.reset();
        ...
        // Now take the previous combined result and add in a removal for uuid 2.
        b = createNewUidList();
        b.setIGNORE(false);
        b.setCOUNT(-1);
        ...
        // and add uuid3. The combined result should still have uuid2 in the remove list, and uuid1 and uuid3 in
        // the uid list.
        b = createNewUidList();
        b.setIGNORE(false);
        b.setCOUNT(1);
    ...
    @Test
    public void testSimpleRemoveAndReAdd() throws Exception {
        agg.reset();
        ...
        // The default behavior is that the remove list trumps, so even though the last thing we did was to
        // add uuid2, since it was already in the remove list that fact trumps and the result should have
        // uuid1 in the UID list and uuid2 in the remove list.
        Collections.reverse(values);
        Value result = agg.reduce(new Key("key"), values.iterator());
        Uid.List resultList = Uid.List.parseFrom(result.get());
        assertEquals(1, resultList.getUIDCount());
    ...
    @Test
    public void testSimpleRemoveAndReAddWithTimestampOrder() throws Exception {
        ...
        GlobalIndexUidAggregator.setProcessInTimestampOrderOpt(is, true);
        ...
        // When the processInTimestampOrder parameter is present, the re-add should take effect
        // and both uuid1 and uuid2 should be in the UID list. Uuid1 should be there because we
    ...
    @Test
    public void testRemoveAndReAddUUIDWithTimestampOrder() throws Exception {
        GlobalIndexUidAggregator localAgg = new GlobalIndexUidAggregator();
        IteratorSetting is = new IteratorSetting(19, "test", GlobalIndexUidAggregator.class);
        GlobalIndexUidAggregator.setCombineAllColumns(is, true);
        GlobalIndexUidAggregator.setProcessInTimestampOrderOpt(is, true);
